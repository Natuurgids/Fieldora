...
try {
    ...
    $renewalConfig = @{
        services = @(
            @{ service_id=$ApiServiceId; organization_id=$Organization; common_name="fieldora"; certificate="/targets/api/service.crt"; private_key="/targets/api/service.key"; dns_names=@("fieldora-server","localhost",$PublicHostname); ip_addresses=@("127.0.0.1",$ListenAddress); uid=$FieldoraUid; gid=$FieldoraGid },
...
Fieldora: https://${PublicHostname}:8765
...
"@
    ...
    Write-Host "Fieldora: https://${PublicHostname}:8765"
    Write-Host "Docs:     https://${PublicHostname}:8765/docs"
    ...
    Pop-Location
}) { throw "PublicHostname is not a valid DNS hostname." }
$parsedListen = $null
if (-not [Net.IPAddress]::TryParse($ListenAddress,[ref]$parsedListen)) { throw "ListenAddress must be an IP address assigned to this host." }
...
"@
    ...
    Pop-Location
}) { throw "PublicHostname must be a DNS hostname." }
$listenIp = $null
if (-not [Net.IPAddress]::TryParse($ListenAddress,[ref]$listenIp)) { throw "ListenAddress must be a valid IP address." }
