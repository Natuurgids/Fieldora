...
| F-03 Local PBAC administration authorization | IN PROGRESS | `AccessAdministrationService` requires authenticated `actor_id` plus PBAC `administer` / `access_control_administration` authorization before organization, identity, group-membership, role-assignment, contract, or policy writes. Contract proposal/approval identities are bound to the actor. Focused hardening workflow run 35073093431 succeeded at `87fd75a03a76d56f807576e7b5ecee9c109b6e08`, including the no-actor, unauthorized-actor, authorized mutation, and Security Install self-provisioning tests. WEB-030 still exposes a real integration defect: the production Project-owner grant path constructs `AccessAdministrationService` without the authenticated creator actor, so project creation fails closed after the F-03 boundary. Fix that production actor propagation without granting general access-policy administration; then wire desktop actor context and Qt read-only behavior. |
...
| F-07 Security certification coverage | IN PROGRESS | Dedicated hardening workflow run 35089520417 succeeded at `e2c1bcf9fbbc0e2885834d16d9e46cf313bc741d`; Qt, browser multi-import, facility-actions, administration-actions, and WEB-060 also succeeded. WEB-030 run 35089519905 still fails in production `_grant_project_owner` because it constructs unauthenticated access administration. This gate remains intentionally failing until the authenticated creator is propagated through the existing narrowly scoped owner-grant boundary. Desktop composition, Qt actor/read-only composition, and immutable runtime-digest gates remain. |

## Current continuation note — 2026-09-16

PR #15 remains open and draft with no comments, review threads, or submitted reviews. `main` remains audited commit `54abf55aa0c5124e1c1e7c46db14814c64a4e37c`. Current code head before this ledger-only commit was `e2c1bcf9fbbc0e2885834d16d9e46cf313bc741d`. Dedicated native-security workflow run 35089520417 succeeded there, certifying the Migration 7 immutability/Migration 8 updater-authority upgrade regression together with the focused native security suite. Qt, browser multi-import, facility-actions, administration-actions, and WEB-060 also succeeded. WEB-030 run 35089519905 remains the only observed failing gate: `BrowserFunctionalityFieldoraApi._grant_project_owner()` still constructs `AccessAdministrationService(self._access_repository)` with no actor and calls general `create_policy()`, which correctly fails closed.

The updater migration history is now upgrade-safe. `MigrationRunner` rejects checksum changes for already-applied migrations, so the previous in-place Migration 7 hardening was invalid for existing installations. Migration 7 has been restored to its historical SQL and Migration 8 carries the cleanup/isolation changes. Do not edit either migration in place again; future changes require a new migration number.

For WEB-030, do not exempt `OBJECT_GRANT` from authenticated actor checks and do not give project creators general `administer/access_policy` authority. `AccessAdministrationService.grant_project_owner_workspace()` already provides the intended narrow seam: it requires an authenticated enabled organizational actor, re-authorizes that actor for `create/project` with research purpose, and can only emit that actor's project-scoped research OBJECT_GRANT. Change the browser production path to instantiate administration with `actor_id=identity_id` and call that narrow operation. WEB-030 must then prove creator immediate access and explicit-deny precedence.

After WEB-030 is green, highest-priority unresolved integration remains desktop composition across F-01/F-03/F-04. `MainWindow` still constructs `OfflineUpdateService()` without the required trust-anchor argument, keeps update state under the library root, launches the updater without explicit config/trust roots, and constructs access administration without authenticated actor context while rediscovering application paths from environment. Continue by wiring the already-resolved `container.paths` and authenticated desktop identity through bootstrap -> `run_desktop` -> `MainWindow`, and make Access & Contracts read-only when that actor lacks administration authority. Large-file GitHub edits require complete-file replacement: do not overwrite `ui/qt/application.py` or `bootstrap/cli.py` from partial fetches.
