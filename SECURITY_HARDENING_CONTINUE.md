...
| F-07 Security certification coverage | IN PROGRESS | At `be1587b792ac1010112057f5d67094bf46923e20`, all 14 observed PR workflow runs completed successfully. Desktop composition and immutable runtime-digest gates remain. |

## Current continuation note — 2026-09-16

PR #15 remains open and draft with no comments, review threads, or submitted reviews. `main` remains `54abf55aa0c5124e1c1e7c46db14814c64a4e37c`. At the latest mandatory re-check, branch head `be1587b792ac1010112057f5d67094bf46923e20` was 54 commits ahead and 0 behind main. All 14 workflow runs observed for that head completed successfully.
...
**Tooling boundary discovered during the latest continuation:** the connected GitHub contents writer only supports complete-file replacement, while connector reads of `src/natureai_next/ui/qt/application.py` and `src/natureai_next/bootstrap/cli.py` are truncated before a complete replacement body can be safely reconstructed. Do not overwrite either large file from those partial reads. Use a patch-capable/local checkout (or another tool that can preserve untouched file bytes) for this composition edit. This is a tooling limitation, not a security-design blocker; do not weaken the explicit-root or actor-context requirements to work around it.

F-06 remains genuinely unimplemented. Existing container-related CI verifies exact checked-out PR head in workflows, which is source identity only; it does not demonstrate the required Git commit -> OCI digest -> deployed digest -> running container digest chain. Do not reclassify existing exact-head checks as runtime identity certification.
