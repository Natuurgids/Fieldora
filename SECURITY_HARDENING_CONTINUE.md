...
| F-07 Security certification coverage | IN PROGRESS | Dedicated hardening workflow run 35076442510 succeeded at `92a71b5334ce6401b77d0871af1d58080e571062`; Qt, browser multi-import, facility-actions, administration-actions, and WEB-060 also succeeded. WEB-030 run 35076442459 failed in the production `_grant_project_owner` path because it still constructs unauthenticated access administration. This gate must remain failing until production propagates the authenticated creator through a narrowly authorized owner-grant boundary. Desktop composition, Qt actor/read-only composition, and immutable runtime-digest gates remain. |

## Current continuation note — 2026-09-16

PR #15 remains open and draft. `main` remains audited commit `54abf55aa0c5124e1c1e7c46db14814c64a4e37c`. At head `92a71b5334ce6401b77d0871af1d58080e571062`, dedicated native-security workflow run 35076442510 succeeded, as did Qt, browser multi-import, facility-actions, administration-actions, and WEB-060. WEB-030 run 35076442459 failed two tests after the fixture repair. The failure is no longer fixture setup: `BrowserFunctionalityFieldoraApi._grant_project_owner()` constructs `AccessAdministrationService(self._access_repository)` with no actor and calls `create_policy()`, which correctly raises `AccessDenied("access-control mutation requires authenticated actor")`. This is a production integration regression introduced by making F-03 fail closed.

Do not bypass this by exempting `OBJECT_GRANT` from authenticated actor checks or by giving every project creator general `administer/access_policy` authority. Preserve least privilege: the Project creation flow already has an authenticated creator and an authorized `create/project` decision; propagate that creator into a narrowly scoped owner-grant operation whose only possible output is the subject-specific, new-project-scoped research OBJECT_GRANT. WEB-030 must prove creator immediate access and explicit-deny precedence after the production fix.
