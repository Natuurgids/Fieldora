...
| --- | --- | --- |
| F-01 Native release authenticity/provenance | IN PROGRESS | Ed25519 signed-index verification, strict signed-field validation, administrator trust-anchor loading, package hash/size and Security Install evidence binding, detached-updater re-verification, cryptographically derived Security Install release context, and signed-index release tooling are implemented. Serialized provenance booleans are non-authoritative. Focused positive/negative authenticity tests exist. Remaining: wire the administrator trust-anchor path through desktop composition and observe the focused security gates in CI. |
| F-02 Native updater PBAC provisioning | IMPLEMENTED — TESTS PENDING | Access-control migration 7 provisions enabled service identity `fieldora-native-updater` and one direct allow policy limited to `install` / `security_install_release` / `security_install`. Tests cover clean install, idempotency, exact policy shape, unrelated-subject denial, and missing/disabled updater denial. Existing broad workflows passed at head `a20457d`, but no dedicated focused security certification has been observed. |
| F-03 Local PBAC administration authorization | NOT STARTED | Contract documented; application-layer actor authorization and fail-closed Qt mutation controls still required. |
| F-04 Config-root binding | IN PROGRESS | Handoff can carry explicit config/trust roots; detached updater requires explicit `--config-root` and `--trust-anchor`, resolves PBAC/history from that root, rejects request/root mismatch, and re-verifies the signed staged index. Handoff tests cover explicit arguments. Remaining: desktop composition must pass the resolved application root/trust anchor and tests must prove conflicting inherited root environment variables cannot redirect updater state. |
| F-05 Authenticated anti-rollback | IMPLEMENTED — TESTS PENDING | Version and minimum-supported-version are consumed only after Ed25519 verification; tamper-negative tests exist. Dedicated focused CI result still required. |
| F-06 Immutable deployment/runtime binding | NOT STARTED | Contract documented; implementation required. |
| F-07 Security certification coverage | IN PROGRESS | Native authenticity, Security Install provenance/PBAC, handoff-root, and updater provisioning tests exist. Dedicated workflow plus desktop env-root, local PBAC administration, and immutable runtime-digest gates remain required. |

## Current continuation note — 2026-09-16

PR #15 is still open and draft. At head `a20457dae3735a1e1773c6acaf57195be1278ab7`, the branch was 21 commits ahead of and 0 behind `main`, whose merge base remained audited commit `54abf55aa0c5124e1c1e7c46db14814c64a4e37c`; there were no PR review comments. Four existing pull-request workflows completed successfully at that head, but the combined commit-status endpoint reported no status contexts and there is still no dedicated security-hardening workflow proving the focused gates. F-02 provisioning is implemented with tests. The highest-priority unresolved integration is F-01/F-04 desktop composition: `MainWindow` still constructs `OfflineUpdateService` without its required administrator trust-anchor argument, stores update settings/staging/history under the library/session root, and launches the detached updater without explicit config/trust roots. Continue by binding desktop update state to the explicit application root and passing the administrator trust anchor through desktop -> stage -> handoff -> updater; then add conflicting-environment-root tests before starting F-03.
