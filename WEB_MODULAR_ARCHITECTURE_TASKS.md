...
## Current dependency findings at audit head `f250f9d2a6aeddfa44e2fae1e6cb3cc363b7a8e3`
...
- Capacity's modular owner is deliberately narrower than the full legacy Capacity page: `capacity_module_web.py` owns the selected-Project allocation view, while the base page still owns schedule/absence/obligation/allocation creation compatibility UI. The Projects→Capacity adapter now synchronizes the surviving legacy `#capacity-project` selector from `projects.context.select` before opening Capacity and whenever Project context is registered or changed, so the normal modular Project→Capacity path no longer depends on `projects[0]` to choose a Project. Focused integration coverage proves that bounded selector bridge while rejecting `projects.list.read` and `projects[0]` in the adapter. Exact-head finalizer inspection still shows no Capacity retirement rule for the legacy creation function or its `capacity-save` wiring, and direct Capacity navigation with an empty selector can still reach the raw `saveCapacity()` first-project fallback. P08 therefore narrows but remains open.
- The legacy Dossier workspace remains outside current modular retirement markers: the registered `research.dossiers` capability owns the modular `/research` route, while the base shell's separate `/dossiers` workspace, `saveDossierWorkspace()` function and `dossier-save` wiring survive finalization. The compatibility boundary now synchronizes the surviving `#dossier-project` selector from `projects.context.select` on Project-context registration/change and after canonical Project-list mirroring rebuilds the selector options. Focused regression coverage proves this bounded bridge and rejects `projects[0]` inside the context-sync helper itself. `saveDossierWorkspace()` still retains `q("dossier-project").value||projects[0]?.id||"platform"` for direct/blank legacy navigation, so the compatibility Project-list mirror remains an active dependency and P08 stays open.
- Platform AI Administration, Reference Data and Connector creation are Project-independent administration surfaces. Their save paths now send the explicit platform scope `project_id:"platform"` instead of deriving a scope from `projects[0]`, removing three unrelated ambient Project-list consumers while preserving the generic resource API/PBAC project-id field. Focused regression coverage also locks the connector list renderer to its `cards(...)` contract after the adjacent rendering correction. Capacity and Dossier first-project fallbacks remain intentionally unchanged, so this narrows P08 debt without closing it.
- Base `projectOptions()` populates ten legacy Project selectors (`upload-project`, `obs-project`, `record-project`, `knowledge-project`, `contract-project`, `device-project`, `work-project`, `capacity-project`, `dossier-project`, `science-project`). Because surviving Capacity, Dossier and work-editor paths still consume members of that set, the compatibility `fieldora:project-list-changed` bridge must continue to mirror the canonical list into ambient `projects` and refresh selectors for now. The Dossier compatibility boundary immediately reapplies `projects.context.select` after that selector rebuild so an existing modular Project context is not silently replaced by list order. Individual selectors should be retired only as their owning capability is migrated or proven stripped from the finalized shell.
...
- Remaining ambient coupling keeps P08 open: the compatibility `projectOptions()` wrapper/`fieldora:project-list-changed` bridge still mirrors the canonical Project list into legacy `projects` for surviving selectors, Project Core still maintains the bounded `#work-project` mirror for the surviving legacy work editor, direct Capacity creation can still fall through to the legacy first-project fallback when no Project context/selector value is present, and direct/blank legacy Dossier creation can still fall through to its first-project fallback even though an available modular Project context is now reapplied to `#dossier-project`. These are now classified active compatibility consumers rather than untraced source occurrences.
