...
## Current dependency findings at audit head `bae246105ee80e747cd40617a70716850403f3aa`
...
- Remaining ambient coupling keeps P08 open: the base bundle still defines and reads mutable `projects` in Home, Research, the transitional `loadPortfolio` renderer and several selector/default-value paths; `loadBase()` still seeds that legacy mirror; `openProject()` and record/export paths still use `selectedProject`; Projects/Core still writes the transitional `selectedProject` and `#work-project` mirrors; Portfolio still reads `window.me` and calls transitional `window.loadPortfolio`; and the compatibility `projectOptions()` wrapper/bridge remains live for surviving legacy selectors.
