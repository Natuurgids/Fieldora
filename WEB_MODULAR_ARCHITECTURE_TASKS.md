...
## Current dependency findings at audit head `ffef88711358cc3d3aee38bf1f96073b57215024`
...
- `DossierModuleWebApiMixin` is composed in the managed API and the modular-shell finalizer retires the legacy `loadDossierWorkspace()` / `saveDossierWorkspace()` range plus the legacy refresh, save and workspace-list `.onclick` handlers only when both the Dossier owner marker and registered manifest entry are present. Production-order final-shell coverage now proves the registered Dossier owner and new `addEventListener` wiring survive while those legacy competitors are absent and unrelated Research loading remains intact.
...
- Platform AI Administration, Reference Data and Connector creation are Project-independent administration surfaces. Their save paths now send the explicit platform scope `project_id:"platform"` instead of deriving a scope from `projects[0]`, removing three unrelated ambient Project-list consumers while preserving the generic resource API/PBAC project-id field. Focused regression coverage also locks the connector list renderer to its `cards(...)` contract after the adjacent rendering correction. Capacity's first-project fallback remains intentionally unchanged, so this narrows P08 debt without closing it.
- Base `projectOptions()` still populates ten legacy Project selectors (`upload-project`, `obs-project`, `record-project`, `knowledge-project`, `contract-project`, `device-project`, `work-project`, `capacity-project`, `dossier-project`, `science-project`). That generic compatibility refresh remains necessary for surviving legacy consumers such as Capacity, the work editor and other selector-backed compatibility surfaces. Although `dossier-project` remains in the generated options set, Dossier no longer uses that selector as save authority; its effective Project scope comes from `projects.context.select`.
...
- Remaining ambient coupling keeps P08 open: the compatibility `projectOptions()` wrapper/`fieldora:project-list-changed` bridge still mirrors the canonical Project list into legacy `projects` for surviving selectors, Project Core still maintains the bounded `#work-project` mirror for the surviving legacy work editor, and direct Capacity creation can still fall through to the legacy first-project fallback when no Project context/selector value is present. Dossier is no longer part of that active ambient-scope debt because its finalized owner saves only from `projects.context.select` and its legacy save path is retired.
