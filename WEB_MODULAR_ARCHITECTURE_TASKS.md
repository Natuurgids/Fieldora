...
## Current dependency findings at audit head `26c59367e8191759412a9a398b48d6aa70cf1c1b`
...
- Remaining ambient coupling keeps P08 open: the base Research workspace still renders its Project list from mutable `projects`; `loadBase()` still seeds the legacy mirror for compatibility; `openProject()` and record/export paths still use `selectedProject`; Projects/Core still writes the transitional `selectedProject` and `#work-project` mirrors; and the compatibility `projectOptions()` wrapper/bridge remains live for surviving legacy selectors.
- Research's bounded Projects integration intentionally requires only `projects.context.select` and `projects.toolbar.extend`: it opens the selected Project through the Research-owned `FieldoraResearchRecords` bridge and does not consume a Project-list contract. The separate legacy Research `#project-list`/`openProject()` surface therefore needs classification as legacy UI before adding any new Research list dependency or retiring its ambient `projects` read.
