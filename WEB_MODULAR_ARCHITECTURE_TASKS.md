...
## Current dependency findings at audit head `4285c9382f2f49ffd81c1d627cb3058645dbe354`
...
- The legacy Dossier workspace is likewise still outside the current modular retirement markers. `saveDossierWorkspace()` and its `dossier-save` wiring survive finalization and still use `q("dossier-project").value||projects[0]?.id||"platform"`. This is a concrete surviving consumer of the compatibility Project-list mirror, not merely dead raw-source text.
...
- Base `projectOptions()` populates ten legacy Project selectors (`upload-project`, `obs-project`, `record-project`, `knowledge-project`, `contract-project`, `device-project`, `work-project`, `capacity-project`, `dossier-project`, `science-project`). Because surviving Capacity, Dossier and work-editor paths still consume members of that set, the compatibility `fieldora:project-list-changed` bridge must continue to mirror the canonical list into ambient `projects` and refresh selectors for now. Individual selectors should be retired only as their owning capability is migrated or proven stripped from the finalized shell.
