...
## Current dependency findings at audit head `c69a17de6a2ef09fa2275f30b03f701bc9b70960`
...
- Managed zero-trust `loadBase()` still calls legacy `projectOptions()` after a successful `projects.list.read.refresh()`, even though the provider always emits `fieldora:project-list-changed` and the compatibility bridge already mirrors the canonical snapshot into legacy `projects` and refreshes selectors. That second call is redundant on the successful contract path, but the same call still clears or populates legacy selectors on denied/missing-contract and no-contract-runtime fallback paths; narrow it only with focused final-shell evidence rather than deleting compatibility selector refresh wholesale.
...
- Remaining ambient coupling keeps P08 open: the compatibility `projectOptions()` wrapper/`fieldora:project-list-changed` bridge still mirrors the canonical Project list into legacy `projects` for surviving selectors, and Project Core still maintains the bounded `#work-project` mirror for the surviving legacy work editor. Exact final-shell tracing is still required before deleting either compatibility surface wholesale.
