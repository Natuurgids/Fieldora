...
## Current dependency findings at audit head `104b5553b3db74fc8cc84f03da471bdba55f720f`
...
- Capacity's modular owner is deliberately narrower than the full legacy Capacity page: `capacity_module_web.py` owns the selected-Project allocation view, while the base page still owns schedule/absence/obligation/allocation creation compatibility UI. Exact-head finalizer inspection shows no Capacity retirement rule for those base functions or their `capacity-save` wiring, so `saveCapacity()` remains an active final-shell consumer. Its empty-selector fallback still reads `projects[0]?.id`; that ambient read cannot be removed until the legacy creation surface is migrated behind an explicit Project-list/context contract or replaced.
...
- Remaining ambient coupling keeps P08 open: the compatibility `projectOptions()` wrapper/`fieldora:project-list-changed` bridge still mirrors the canonical Project list into legacy `projects` for surviving selectors, Project Core still maintains the bounded `#work-project` mirror for the surviving legacy work editor, and the surviving Capacity/Dossier creation paths still consume the legacy list as a first-project fallback. These are now classified active compatibility consumers rather than untraced source occurrences.
