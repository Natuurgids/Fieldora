...
## Current dependency findings at audit head `77552a49555c0b0b2799ee47dd7a29c99d3cba87`
...
- Remaining ambient coupling keeps P08 open: the base bundle still defines and reads mutable `projects` in Home, Research and several selector/default-value paths; `loadBase()` still seeds that legacy mirror; `openProject()` and record/export paths still use `selectedProject`; Projects/Core still writes the transitional `selectedProject` and `#work-project` mirrors; and the compatibility `projectOptions()` wrapper/bridge remains live for surviving legacy selectors.
