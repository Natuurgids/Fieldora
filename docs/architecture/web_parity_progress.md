...
**Iteration 6 evidence:** `src/natureai_next/server/project_core_module_web.py` owns a dedicated Projects work hierarchy surface backed directly by the governed `/api/v1/phases`, `/api/v1/tasks`, `/api/v1/sprints`, and `/api/v1/allocations` endpoints rather than Portfolio DOM datasets. `src/natureai_next/server/project_work_actions_module_web.py` now owns the visible creation flow for phases, tasks, milestones, subtasks, sprints and allocations. It validates required names/titles, allocation ranges and date ordering, projects edit capability only for discoverability, and emits `fieldora:project-work-changed` after successful persistence so Projects/Core reloads its own hierarchy. `web_module_contracts.py` assigns `projects.phase.create`, `projects.task.create`, `projects.milestone.create`, `projects.subtask.create`, `projects.sprint.create`, and `projects.allocation.create` to `projects.core`. The managed hierarchy API remains the independent authorization boundary: it authenticates the caller, evaluates PBAC for both the Project and child resource, then persists through Project Management. `modular_shell_web.py` retires the former WEB-032 browser owner when Projects/Core is present without removing that server API. `tests/test_project_core_module_web.py` and `tests/test_project_work_actions_module_web.py` cover the Projects-owned hierarchy/actions and final composition. Top-level Project create/edit lifecycle, status/progress ownership, browser/final-DOM certification and full desktop parity remain outstanding, so no Projects requirement is marked complete yet.
