...
AI Review opens immediately and shows a loading message while Aperture retrieves model status, queue counts, and the first page of suggestions in the background. Selecting a suggestion also loads its evidence, personal observation context, ecology, and enrichment history without blocking the rest of the application. Changing filters during a load is safe: Aperture finishes the active query and then applies the newest requested filter.
