...
Before a download starts, Fieldora shows:
...
When a complete prebuilt collection is available, use **Import Map Bundle** instead of downloading its regions individually. Fieldora accepts `.apkg` map bundles containing:

- `bundle.json`;
- one or more MBTiles files;
- a SHA-256 checksum and package metadata for every file.

Fieldora verifies every package before registration. Invalid, incomplete, unsafe, or checksum-mismatched bundle contents are not activated.
...
If no enabled package covers the current view, Fieldora keeps the Map workspace available and reports that offline map coverage is unavailable.
...
Fieldora does not create offline archives by bulk-downloading from public
OpenStreetMap or OpenSeaMap tile servers. Catalogs, bundles and nautical imports
must provide licensed, prebuilt Fieldora-compatible MBTiles packages with
checksums, attribution and licence metadata.


## Geofabrik OpenStreetMap provider

Fieldora uses the official Geofabrik region index as its default online map source. An internet connection is required to retrieve the catalog and selected regional `.osm.pbf` extract. Only leaf regions offered by the provider are downloadable in the normal interface. Fieldora verifies the provider file when a checksum sidecar is available, converts it locally into PMTiles through base zoom 14, and registers the finished package. MapLibre overzooms the vector data for street-level interaction. Installed maps then work without a network connection.
...
New Geofabrik selections use `.osm.pbf` vector input and require the verified Fieldora vector converter toolchain before download begins. Fieldora no longer silently prepares a new zoom-10 raster map when street-level output was requested. Existing raster packages remain available as compatibility maps.

## Build 3 field validation

Validate both routes before approving the increment:

1. In Fieldora, open **Resources → Offline Maps → Open Offline Map Setup…**.
...
Selected regions are queued as separate background activities. You can close the Offline Maps window and continue using Fieldora. Open **Activity Center** to see the current phase, download speed, bytes transferred, rendered tile count, completion, cancellation, or retry. Progress is refreshed once per second to avoid slowing the transfer or map creation.
...
Choose **All** to combine every compatible installed vector MBTiles region in one map. Fieldora resolves each requested vector tile against the installed packages whose bounds and zoom range cover that tile. Choose a named area to restrict rendering to that package only.
