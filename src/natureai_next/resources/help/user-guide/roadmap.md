# Fieldora Product Roadmap

**Status:** Directional roadmap for post-Version 1 development  
**Scope:** Fieldora product, Fieldora Engine engine, documentation, and supporting tools  
**Dates:** Deliberately not assigned. Priorities may change after field use and user feedback.

## Guiding principles

Fieldora is designed as a long-lived, offline-capable scientific platform. Development follows these principles:
...
- Fieldora desktop application powered by the `Fieldora Engine` engine.
- BioCLIP-assisted identification and AI review workflows.
- Managed Fieldora Library using SQLite.
...
Fieldora Engine will evolve from a single-model integration into a modular orchestrator. BioCLIP remains an important engine, while additional task-specific models may be loaded only when required. Planned categories include taxonomy classification, behavior, habitat, segmentation, quality assessment, similarity, OCR, and future specialist engines.
...
- Lessons and suitable approaches may be studied from established open-source digital asset managers such as digiKam, subject to license compatibility and independent Fieldora architecture decisions.
...
Fieldora may suggest evidence needed to improve an observation, such as:
...
Fieldora treats the user’s library as the most valuable part of the system. Future releases will provide documented migrations, validation, backups, and rollback paths for schema or metadata changes whenever practical.
