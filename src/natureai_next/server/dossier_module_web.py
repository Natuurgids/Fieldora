...
(()=>{
 ...
 function renderLifecycle(){const panel=q("dossier-lifecycle-panel"),dossier=selectedDossier();if(!panel)return;if(!dossier){panel.hidden=true;return}panel.hidden=false;q("dossier-lifecycle-name").value=String(dossier.name||dossier.title||"");q("dossier-lifecycle-description").value=String(dossier.description||"");q("dossier-lifecycle-type").value=dossier.dossier_type==="master"?"master":"dossier";q("dossier-lifecycle-owner").value=String(dossier.owner_id||dossier.created_by||"");q("dossier-lifecycle-reviewer").value=String(dossier.reviewer_id||"");q("dossier-lifecycle-remark").value="";lifecycleStatus(`Review state: ${dossier.review_status||dossier.status||"draft"}`)}
