...
(()=>{
 ...
 function ensureLifecycleSurface(){const page=q("page-dossiers");if(!page||q("dossier-lifecycle-panel"))return;const panel=document.createElement("section");panel.id="dossier-lifecycle-panel";panel.className="card section";panel.hidden=true;panel.innerHTML='<h2>Dossier lifecycle</h2><p class="muted">Edit governed Dossier metadata or move the selected Dossier through its review handoff. Server authorization remains authoritative.</p><label>Name<input id="dossier-lifecycle-name" type="text"></label><label>Description<textarea id="dossier-lifecycle-description" rows="3"></textarea></label><label>Dossier type<select id="dossier-lifecycle-type"><option value="dossier">Dossier</option><option value="master">Master dossier</option></select></label><p id="dossier-lifecycle-project" class="muted"></p><div class="actions section"><button id="dossier-lifecycle-use-project" type="button">Move to current Project</button><button id="dossier-lifecycle-independent" type="button">Make independent</button></div><div class="actions section"><button id="dossier-lifecycle-update" class="primary" type="button">Save changes</button></div><label>Observer/owner identity<input id="dossier-lifecycle-owner" type="text"></label><div class="actions section"><button id="dossier-lifecycle-reassign-owner" type="button">Reassign owner</button></div><label>Reviewer identity<input id="dossier-lifecycle-reviewer" type="text"></label><label>Review remark<textarea id="dossier-lifecycle-remark" rows="3"></textarea></label><div class="actions section"><button id="dossier-lifecycle-defer" type="button">Defer for review</button><button id="dossier-lifecycle-remark-action" type="button">Add reviewer remark</button><button id="dossier-lifecycle-return" type="button">Return to observer</button></div><p id="dossier-lifecycle-status" class="status"></p>';q("dossier-workspace-detail")?.after(panel)}
 ...
 function renderLifecycle(){const panel=q("dossier-lifecycle-panel"),dossier=selectedDossier();if(!panel)return;if(!dossier){panel.hidden=true;return}panel.hidden=false;q("dossier-lifecycle-name").value=String(dossier.name||dossier.title||"");q("dossier-lifecycle-description").value=String(dossier.description||"");q("dossier-lifecycle-type").value=dossier.dossier_type==="master"?"master":"dossier";q("dossier-lifecycle-project").textContent=`Project: ${dossier.project_id||"Independent"}`;q("dossier-lifecycle-owner").value=String(dossier.owner_id||dossier.created_by||"");q("dossier-lifecycle-reviewer").value=String(dossier.reviewer_id||"");q("dossier-lifecycle-remark").value="";lifecycleStatus(`Review state: ${dossier.review_status||dossier.status||"draft"}`)}
 ...
 async function updateSelectedDossier(){const dossier=selectedDossier();if(!dossier)return;try{const service=dataService();if(!service?.updateDossier)throw new Error("Dossier data service is unavailable.");const result=await service.updateDossier(dossier.id,{name:q("dossier-lifecycle-name")?.value||"",description:q("dossier-lifecycle-description")?.value||"",dossier_type:q("dossier-lifecycle-type")?.value||"dossier"});await applyLifecycleResult(result,"updated");lifecycleStatus("Dossier changes saved.")}catch(error){lifecycleStatus(error?.message||"Dossier could not be updated.",true)}}
 async function updateSelectedDossierProject(projectId,message){const dossier=selectedDossier();if(!dossier)return;try{const service=dataService();if(!service?.updateDossier)throw new Error("Dossier data service is unavailable.");const result=await service.updateDossier(dossier.id,{project_id:String(projectId||"")});await applyLifecycleResult(result,"project_changed");lifecycleStatus(message)}catch(error){lifecycleStatus(error?.message||"Dossier Project could not be changed.",true)}}
 async function moveSelectedDossierToCurrentProject(){const projectId=syncProjectContext();if(!projectId){lifecycleStatus("Select a Project before moving the dossier.",true);return}await updateSelectedDossierProject(projectId,"Dossier moved to the current Project.")}
 async function makeSelectedDossierIndependent(){await updateSelectedDossierProject("","Dossier is now independent.")}
