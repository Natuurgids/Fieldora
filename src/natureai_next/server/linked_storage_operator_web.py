...
(()=>{
 ...
 async function enrollStorageService(){
  ...
  if(!name||!nodeName||!serial||!expiryEpoch){if(status)status.textContent="Service name, node name, certificate serial and certificate expiry are required.";return}
  if(status)status.textContent="Enrolling durable linked-storage service identity…";
  try{
   const created=await api("/api/v1/operator/services",{method:"POST",purpose:"administration",body:JSON.stringify({name,service_type:"linked-storage",node_name:nodeName,software_version:"",configuration_sha256:"",certificate_serial:serial,certificate_not_after_epoch:expiryEpoch})});
   const service=created.service;
   if(!service?.service_id)throw new Error("Server did not return an enrolled service identity.");
 ...
 async function loadOperatorExtensions(){
  ...
  try{
   ...
   }).join(""):'<div class="empty">No linked archives registered for this organization. Enroll and activate a linked-storage service below, then complete archive registration from that storage node.</div>';
 ...
 function enhanceOperator(){
  ...
  if(!byId("operator-linked-archives")){
   ...
   section.innerHTML='<h2>Linked archives</h2><p class="muted">Archive ownership, enrolled storage service and heartbeat freshness. Disablement revokes Library access without exposing or changing storage-node paths.</p><p id="operator-linked-archives-status" class="status"></p><div id="operator-linked-archives" class="list"></div><h3>Connect a storage service</h3><p class="muted">Enroll only the durable service identity here. Filesystem roots, aliases, private keys, CA certificates and storage credentials stay on the storage node. The storage node registers the read-only archive after mTLS activation.</p><div class="form-grid"><label>Service name<input id="operator-linked-service-name" autocomplete="off" placeholder="Herbarium archive service"></label><label>Node name<input id="operator-linked-service-node" autocomplete="off" placeholder="archive-node-01"></label><label>Certificate serial<input id="operator-linked-service-certificate-serial" autocomplete="off"></label><label>Certificate expiry<input id="operator-linked-service-certificate-expiry" type="date"></label></div><div class="actions section"><button id="operator-linked-service-enroll" class="primary" type="button">Enroll linked-storage service</button></div><p id="operator-linked-service-setup-status" class="status"></p><div id="operator-linked-service-handoff" class="section" hidden></div><h3>Recent lifecycle activity</h3><p class="muted">Newest registration and Operator lifecycle events for this organization.</p><div id="operator-linked-archive-events" class="list"></div>';
