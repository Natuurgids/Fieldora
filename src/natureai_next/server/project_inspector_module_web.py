...
(()=>{
 ...
 function ensureSurface(){
  ...
  panels.forEach(([key,label])=>{
   ...
   if(key==="properties")panel.innerHTML='<div id="project-inspector-properties-content" class="empty">Select a project or work item.</div>';
 ...
 function render(selection){
  ensureSurface();const record=selection?.record||null,properties=q("project-inspector-properties-content"),metadata=q("project-inspector-metadata"),map=q("project-inspector-map"),activity=q("project-inspector-activity"),title=q("project-cockpit-title");
  if(title)title.textContent=record?.name||record?.title||"Project workspace";
  if(!record){if(properties){properties.className="empty";properties.innerHTML="Select a project or work item."}if(metadata)metadata.innerHTML='<div class="empty">Select a project or work item.</div>';if(map)map.innerHTML='<div class="empty">Select a project to inspect its spatial context.</div>';if(activity)activity.innerHTML='<div class="empty">Select a record.</div>';return}
  const kind=String(selection?.kind||"record");
  if(properties){properties.className="";properties.innerHTML=`<h3>${esc(record.name||record.title||record.id)}</h3><p><strong>Type</strong> ${esc(kind)}</p><p><strong>Status</strong> ${esc(record.status||"active")}</p><p><strong>Owner</strong> ${esc(record.owner_id||record.assignee_id||record.manager_id||"—")}</p><p class="muted">${esc(record.description||record.notes||"No description recorded.")}</p>`}
  if(metadata)metadata.innerHTML=`<h3>${esc(record.name||record.title||record.id)}</h3><pre>${esc(JSON.stringify(record,null,2))}</pre>`;
  if(map)map.innerHTML=`<div class="project-map-stage"><h3>Project map</h3><p>${esc(record.name||record.title||record.id)}</p><p class="muted">${esc(record.research_area||record.location||record.geography||"No spatial boundary has been recorded for this project yet.")}</p><p class="muted">Map packages remain governed by Fieldora map installation and offline-map services.</p></div>`;
