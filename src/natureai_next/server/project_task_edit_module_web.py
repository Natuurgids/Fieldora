...
(()=>{
 ...
 const state={mounted:false,controller:null,projectId:"",taskId:"",pendingTaskId:"",canEdit:false};
 ...
 async function open(taskId){
  if(!state.projectId||!taskId||!state.canEdit)return;state.taskId=taskId;const service=workData();if(!service?.taskDetail||!service?.statuses||!service?.phases||!service?.sprints){state.pendingTaskId=taskId;return}
  try{
   const [task,statuses,phases,sprints]=await Promise.all([service.taskDetail(state.projectId,taskId),service.statuses(state.projectId),service.phases(state.projectId),service.sprints(state.projectId)]);
   state.pendingTaskId="";if(!task)throw new Error("Task is no longer available.");
 ...
 async function save(){
  ...
  const service=workData();if(!service?.updateTask)return fail("Project work-data contract is unavailable.");
  try{const item=await service.updateTask(state.projectId,state.taskId,record);q("project-core-task-editor").hidden=true;document.dispatchEvent(new CustomEvent("fieldora:project-work-changed",{detail:{module_id:moduleId,project_id:state.projectId,kind:"task",item}}))}catch(error){fail(error?.message||"Task could not be saved.");emitError(error,"Task could not be saved.")}
 }
 ...
 function unmount(){state.controller?.abort();state.controller=null;state.mounted=false;state.pendingTaskId="";const host=q("project-core-task-editor");if(host)host.hidden=true}
 document.addEventListener("fieldora:project-context-changed",()=>{state.projectId=projectContext()?.current?.()||"";authority()});document.addEventListener("fieldora:contract-registered",event=>{if(event.detail?.contract==="projects.list.read"&&state.mounted)authority();if(event.detail?.contract==="projects.work-data.service"&&state.mounted&&state.pendingTaskId)open(state.pendingTaskId)});document.addEventListener("fieldora:module-mount",event=>{if(event.detail?.module?.module_id===moduleId)mount()});document.addEventListener("fieldora:module-unmount",event=>{if(event.detail?.module?.module_id===moduleId)unmount()});
