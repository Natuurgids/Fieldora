...
(()=>{
 ...
 async function open(taskId){
  if(!state.projectId||!taskId||!state.canEdit)return;state.taskId=taskId;const pid=encodeURIComponent(state.projectId);
  try{
...
class ProjectTaskEditModuleWebApiMixin:
    ...
    def dispatch(self, method: str, target: str, headers: dict[str, str], body: bytes) -> ApiResponse:
        ...
        elif service is not None and route.path.startswith("/api/v1/tasks/") and method == "PATCH":
            response = self._patch_task(route.path.rsplit("/", 1)[-1], routed_headers, body)
        else:
            response = super().dispatch(method, target, headers, body)
        browser_session = getattr(self, "_browser_session_response", None)
        if callable(browser_session) and route.path.startswith("/api/v1/") and method in {"GET", "PATCH"}:
