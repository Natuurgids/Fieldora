...
(()=>{
 ...
 function openEditor(kind){
  ...
  if(kind==="task"||kind==="milestone"||kind==="subtask"){
   const phaseId=state.selectedWork.kind==="phase"?state.selectedWork.id:"",parentTaskId=kind==="subtask"?state.selectedWork.id:"";
   fields=`<label>Title<input id="project-core-child-title" autocomplete="off"></label><label>Description<textarea id="project-core-child-description"></textarea></label><label>Owner<input id="project-core-child-owner" autocomplete="off"></label><label>Due date<input id="project-core-child-due" type="date"></label><label>Phase ID<input id="project-core-child-phase" value="${escWork(phaseId)}" autocomplete="off"></label><label>Parent task ID<input id="project-core-child-parent" value="${escWork(parentTaskId)}" autocomplete="off"></label><label>Sprint ID<input id="project-core-child-sprint" autocomplete="off"></label><label>Manual estimate (hours)<input id="project-core-child-estimate" type="number" min="0" step="0.25" value="0"></label><label>Realized hours<input id="project-core-child-realized" type="number" min="0" step="0.25" value="0"></label>`;
  }
 ...
 async function saveChild(){
  ...
  if(kind==="phase"){
   ...
   record.title=q("project-core-child-title").value.trim();record.description=q("project-core-child-description").value.trim();record.owner_id=q("project-core-child-owner").value.trim();record.due_date=q("project-core-child-due").value;record.phase_id=q("project-core-child-phase").value.trim();record.parent_task_id=q("project-core-child-parent").value.trim();record.sprint_id=q("project-core-child-sprint").value.trim();record.manual_estimate=Number(q("project-core-child-estimate").value||0);record.realized=Number(q("project-core-child-realized").value||0);if(!record.title)return fail("Task title is required.");if(kind==="subtask"&&!record.parent_task_id)return fail("Parent task is required for a subtask.");if(!Number.isFinite(record.manual_estimate)||record.manual_estimate<0)return fail("Manual estimate must be zero or greater.");if(!Number.isFinite(record.realized)||record.realized<0)return fail("Realized hours must be zero or greater.");
