...
(()=>{
 ...
 const brokerLifecycle=Object.freeze(["requested","acquiring / receiving","quarantined","scanning","verifying","approved","broadcast","claimed","transferred","collector-verifying","accepted"]);
 const section=document.createElement("section");section.id="secure-acquisition";section.className="card section";
 section.innerHTML=`<h2>Secure acquisition</h2><p class="muted">FieldoraBastion is the secure transfer broker for external packages. It accepts collection requests and delivery requests, then quarantines, scans, verifies and approves the package before publishing a bounded broadcast for an eligible collector. Fieldora independently verifies the signed manifest and exact package digests after transfer before trusted-side acceptance. The browser cannot mark content clean or bypass acceptance policy.</p><div class="row"><div><strong>Broker lifecycle</strong><br><span class="muted">${brokerLifecycle.map(esc).join(" → ")}</span></div></div><p class="muted">A broadcast advertises metadata only; package bytes stay in approved Bastion storage until an authenticated collector claims them. A transfer is not accepted until the collector recomputes the digest. Any expected-versus-observed hash discrepancy fails closed as integrity-failed and emits a high-severity package_integrity_mismatch event to the configured security monitoring host. Connected request/status APIs are not assumed here; air-gap transfers preserve the same state and can return the bounded discrepancy event on the next approved channel.</p><div id="secure-acquisition-summary" class="list"></div><p id="secure-acquisition-status" class="status"></p>`;
