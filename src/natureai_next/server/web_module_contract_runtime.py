...
def _runtime_script(registry: WebModuleRegistry | None = None) -> bytes:
    ...
    return (
        ...
        "  document.querySelectorAll('[data-operations-domain]').forEach(button=>{button.onclick=()=>{document.querySelectorAll('[data-operations-domain]').forEach(item=>item.classList.toggle('primary',item===button));return implementation.selectDomain(button.dataset.operationsDomain);};});\n"
        " }\n"
