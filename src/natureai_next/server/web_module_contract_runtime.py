...
def _runtime_script(registry: WebModuleRegistry | None = None) -> bytes:
    ...
    return (
        ...
        "  loadOperations=async function(){const result=await baseOperationsLoad();const snapshot=Object.freeze({domain:currentDomain()});operationsWorkspaceListeners.forEach(listener=>{try{listener(snapshot)}catch(error){console.error('Operations workspace subscriber failed',error)}});return result;};\n"
        ...
        "  register('operations.workspace.host','application.operations-workspace',Object.freeze({currentDomain,selectDomain,refresh:()=>loadOperations(),subscribe}));\n"
        " }\n"
