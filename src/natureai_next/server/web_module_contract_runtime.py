...
def _runtime_script(registry: WebModuleRegistry | None = None) -> bytes:
    ...
    return (
        ...
        "  const operationsDomainButtons=[...document.querySelectorAll('[data-operations-domain]')];\n"
        "  const projectDomainSelection=()=>{const selected=currentDomain();operationsDomainButtons.forEach(item=>{const active=String(item.dataset.operationsDomain||'')===selected;item.classList.toggle('primary',active);item.setAttribute('aria-selected',String(active));item.setAttribute('role','tab');});};\n"
        "  const baseOperationsLoad=loadOperations;\n"
        "  loadOperations=async function(){if(typeof operationsDomain!=='undefined')operationsDomain=operationsWorkspaceDomain;const result=await baseOperationsLoad();operationsWorkspaceRecords=freezeRecords(result);projectDomainSelection();const snapshot=Object.freeze({domain:currentDomain(),records:operationsWorkspaceRecords});operationsWorkspaceListeners.forEach(listener=>{try{listener(snapshot)}catch(error){console.error('Operations workspace subscriber failed',error)}});return result;};\n"
        "  const selectDomain=async domain=>{const next=token(domain);if(!next)throw new Error('Operations workspace domain is required');operationsWorkspaceDomain=next;projectDomainSelection();return loadOperations();};\n"
        ...
        "  projectDomainSelection();\n"
        "  const refreshButton=document.getElementById('operations-refresh');if(refreshButton)refreshButton.onclick=()=>implementation.refresh();\n"
        "  operationsDomainButtons.forEach(button=>{button.onclick=()=>implementation.selectDomain(button.dataset.operationsDomain);});\n"
        " }\n"
