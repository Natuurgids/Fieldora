...
@pytest.mark.integration
def test_project_edit_conflict_and_archive_use_revisioned_managed_api(tmp_path: Path) -> None:
    organization_id = f"browser-lifecycle-org-{uuid4()}"
    with _managed_browser_server(
        organization_id, tmp_path / "access-control-lifecycle.sqlite3"
    ) as (
        url,
        project_management,
        _decisions,
        ...
        _open_project_editor(page, url)
        page.locator("#portfolio-project-name").fill("Lifecycle Browser Project")
        with page.expect_response(
            lambda response: response.request.method == "POST"
            and response.url.endswith("/api/v1/projects")
        ) as create_info:
            page.locator("#portfolio-project-save").click()
        created = create_info.value.json()["item"]
        project_id = created["id"]
        page.locator(f'[data-project-tree="{project_id}"]').click()

        page.locator("#portfolio-edit-project").click()
        page.locator("#portfolio-project-lifecycle-editor").wait_for(state="visible")
        page.locator("#portfolio-project-edit-name").fill("Lifecycle Browser Project Edited")
        page.locator("#portfolio-project-edit-budget").fill("375.25")
        with page.expect_response(
            lambda response: response.request.method == "PATCH"
            and response.url.endswith(f"/api/v1/projects/{project_id}")
        ) as edit_info:
            page.locator("#portfolio-project-edit-save").click()
        edited_response = edit_info.value
        edited = edited_response.json()["item"]
        assert edited_response.status == 200
        assert edited["name"] == "Lifecycle Browser Project Edited"
        assert edited["budget"] == 375.25
        assert edited["revision"] > created["revision"]
        page.locator(f'[data-project-tree="{project_id}"]').get_by_text(
            "Lifecycle Browser Project Edited", exact=True
        ).wait_for(state="visible")

        page.locator("#portfolio-edit-project").click()
        page.locator("#portfolio-project-edit-name").fill("Stale Browser Overwrite")
        concurrent_revision = project_management.update_project(
            project_id,
        ...
        page.locator("#portfolio-project-lifecycle-status").get_by_text(
            "Project changed on the server. Latest values reloaded.", exact=True
        ).wait_for(state="visible")
        assert page.locator("#portfolio-project-edit-name").input_value() == "Concurrent Winner"
        current = next(
            item for item in project_management.projects(organization_id) if item.project_id == project_id
        )
        assert current.revision == concurrent_revision
        assert current.name == "Concurrent Winner"

        page.locator("#portfolio-project-lifecycle-cancel").click()
        page.locator("#portfolio-project-lifecycle-editor").wait_for(state="hidden")
        page.locator("#portfolio-archive-project").click()
        with page.expect_response(
            lambda response: response.request.method == "PATCH"
            and response.url.endswith(f"/api/v1/projects/{project_id}/archive")
        ) as archive_info:
            page.locator("#portfolio-project-archive-confirm").click()
        archived = archive_info.value.json()["item"]
        assert archive_info.value.status == 200
        assert archived["status"] == "archived"
        assert archived["revision"] > concurrent_revision
        assert len(project_management.projects(organization_id)) == 1
        assert [item["event_type"] for item in project_management.activity(project_id)] == [
            "project.created",
            "project.updated",
            "project.updated",
            "project.archived",
