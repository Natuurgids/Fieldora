...
def _write_signed_update(tmp_path: Path, *, signing_key: Ed25519PrivateKey | None = None) -> tuple[Path, Path]:
    ...
    trust.write_text(json.dumps({
        "format": "natureai-next.update-trust-anchors",
        "format_version": 1,
        "keys": {"release-2026": base64.b64encode(public).decode("ascii")},
    }), encoding="utf-8")
    signature = key.sign(canonical_json_bytes(payload))
    (source / "update-index.json").write_text(json.dumps({
        "format": "natureai-next.signed-update",
        "format_version": 1,
        "key_id": "release-2026",
        "signed": payload,
...
def test_untrusted_signer_is_rejected(tmp_path: Path) -> None:
    ...
    trust.write_text(json.dumps({
        "format": "natureai-next.update-trust-anchors",
        "format_version": 1,
        "keys": {"other": base64.b64encode(other).decode("ascii")},
    }), encoding="utf-8")
...
def test_package_size_mismatch_is_rejected(tmp_path: Path) -> None:
    source, trust = _write_signed_update(tmp_path)
    package = source / "fieldora-5.5.0.bin"
...
def test_security_install_evidence_digest_mismatch_is_rejected(tmp_path: Path) -> None:
    ...
    envelope["signed"]["security_install"]["release_id"] = "tampered-evidence"
